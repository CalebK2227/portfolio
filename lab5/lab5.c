 ...
 *
 * @details calls functions in array.c and finds the size, the max, the min,
 * the midpoint, count less than the midpoint, count greater than the midpoint,
 * the midpoint linear search, the sum, the average, the median, then number
 * of even elements, the number of odd elements, and the number of 
 * elements divisible by the min value. these are then printed out to the user.
 *
 * @bugs none
 * 
 * @todo none
 ...
 * @param cnt count of array less than certain value
 * @param less count of array less than midpoint
 * @param greater count of array greater than midpoint
 * @param index tells if the midpoint is in the array
 * @param sum sum of the values in the array
 * @param ave average of the values in the array
 * @param med median of the values in the array
 * @param even count of values in the array that are even
 * @param odd count of values in the array that are odd
 * @param divisible count of values divisible by the minimum
 * @return zero
 */
...
struct summary_t get_summary(int a[], struct summary_t summary);
...
struct summary_t get_summary(int a[], struct summary_t summary)
{
    ...
    summary.cnt = get_count(a, summary.size, LE, 5);
    summary.less = get_count(a, summary.size, LT, summary.mid);
    summary.greater = get_count(a, summary.size, GT, summary.mid);
    summary.index = linear_search(a, summary.size, summary.mid);
    summary.sum = sum_element(a, summary.size);
    summary.ave = ave_sum(a, summary.size, summary.sum);
    summary.med = median(a, summary.size);
    summary.even = even_count(a, summary.size);
    summary.odd = odd_count(a, summary.size);
...
/**
 * prints out all of the results of the functions to the user
 * @return nothing
 */
void print_summary(struct summary_t summary) 
{
        ...
        printf("midpoint of the max and min=%d\n", summary.mid);
        printf("the count is=%d\n", summary.cnt);
        printf("the count of items less than %d is %d\n", summary.mid, summary.less);
        printf("the count of items greater than %d is %d\n", summary.mid, summary.greater);
        if (summary.index != -1) {
                printf("The midpoint was found in the list\n");
        }
        else {
                printf("The midpoint was not found in the list\n");
        }
        printf("The sum of the elements in the array=%d\n", summary.sum);
        printf("The average of the array=%d\n", summary.ave);
        printf("The median of the array=%d\n", summary.med);
        printf("the count of even numbers in the array=%d\n", summary.even);
        printf("the count of odd numbers in the array=%d\n", summary.odd);
        printf("the count of numbers divisible by %d is %d\n", summary.min, summary.divisible);
}