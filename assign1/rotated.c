 ...
 * @param pivot the place where the array is rotated.
...
int main(void){
    ...
    last = n - 1;
    middle = (first + last) / 2;
    printf("Enter %d numbers in a rotated style.\n", n);
    for (i = 0; i < n; i++){
        scanf("%d", &array[i]);
    }
    ...
    place = binary((pivot + 1), last, (((pivot + 1) + last) / 2), array, target, index, max);
    if (place == -1){
        place = binary(first, pivot, ((first + pivot) / 2), array, target, index, max);
        if (place == -1){
